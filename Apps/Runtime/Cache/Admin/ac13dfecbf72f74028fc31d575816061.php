        ...
        <div id="contentwrapper" class="contentwrapper">
            ...
            <div class="tableoptions">
                <form method="GET" action="">
                    <label><input type="text" name="number" id="number" placeholder="设备编码" value="<?php echo ($number); ?>" /></label> 
                    &nbsp;
                    <button type="submit">查询</button>
                    <button type="button" onclick="window.location.href='/easy-city/index.php/Admin/Machine/view'">添加机器</button>
                </form>
            </div>

            <!--tableoptions-->	

            <form name="form" id="form" method="POST" action="/easy-city/index.php/Admin/Machine/del">
                <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablecb" id="tab">
                    ...
                    <?php if(!empty($rows)): if(is_array($rows)): $k = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$i): $mod = ($k % 2 );++$k;?><tr>
                            <td align="center">
                                <input type="checkbox" name="id[]" value="<?php echo ($i["deviceId"]); ?>" />
                            </td>
                            <td>
                                <?php if(($p) == ""): echo ($k); ?>
                                <?php else: ?>
                            ...
                            <td title="<?php echo ($i["deviceNo"]); ?>">
                                <?php echo (msubstr($i["deviceNo"],0,10,'utf-8',true)); ?>
                            </td>
                            <td class="center">
                                <a href="/easy-city/index.php/Admin/Machine/record/id/<?php echo ($i["deviceId"]); ?>">消费流水</a> / <a href="/easy-city/index.php/Admin/Machine/view/id/<?php echo ($i["deviceId"]); ?>">编辑</a> / <a href="/easy-city/index.php/Admin/Machine/del/id/<?php echo ($i["deviceId"]); ?>" onclick="return confirm('删除后会将现有的消费记录一并删除,确定要删除吗?')">删除</a></td>
                        </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                    <?php else: ?>
                        <tr><td colspan="4" align="center">暂无信息...</td></tr><?php endif; ?>
                    </tbody>
                </table>
