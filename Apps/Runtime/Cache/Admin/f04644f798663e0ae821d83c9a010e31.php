        ...
        <div id="contentwrapper" class="contentwrapper">
            ...
            <div class="tableoptions">
                <form method="GET" action="">
                    <label><input type="text" name="title" id="title" placeholder="标题" value="<?php echo ($title); ?>" /></label> 
                    &nbsp;
                    <button type="submit">查询</button>
                    <button type="button" onclick="window.location.href='/easy-city/index.php/Admin/News/view'">添加新闻</button>
                </form>
            </div>

            <!--tableoptions-->	

            <form name="form" id="form" method="POST" action="/easy-city/index.php/Admin/News/del">
                <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablecb" id="tab">
                    ...
                    <?php if(!empty($rows)): if(is_array($rows)): $k = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$i): $mod = ($k % 2 );++$k;?><tr>
                            <td align="center">
                                <input type="checkbox" name="id[]" value="<?php echo ($i["articleId"]); ?>" />
                            </td>
                            <td>
                                <?php if(($p) == ""): echo ($k); ?>
                                <?php else: ?>
                            ...
                            <td title="<?php echo ($i["articleTitle"]); ?>">
                                <?php echo (msubstr($i["articleTitle"],0,10,'utf-8',true)); ?>
                            </td>
                            <td>
                                <?php echo ($i["addDate"]); ?>
                            </td>
                            <td class="center">
                                <a href="/easy-city/index.php/Admin/News/view/id/<?php echo ($i["articleId"]); ?>">编辑</a> / <a href="/easy-city/index.php/Admin/News/del/id/<?php echo ($i["articleId"]); ?>" onclick="return confirm('确定要删除吗?')">删除</a></td>
                        </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                    <?php else: ?>
                        <tr><td colspan="5" align="center">暂无信息...</td></tr><?php endif; ?>
                    </tbody>
                </table>
